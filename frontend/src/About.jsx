...
function About() {
    ...
    useEffect(() => {
        let ctx = gsap.context(() => {
            const xMask = gsap.quickTo(sectionRef.current, "--mouse-x", { duration: 0.3, ease: "power3.out", unit: "px" });
            const yMask = gsap.quickTo(sectionRef.current, "--mouse-y", { duration: 0.3, ease: "power3.out", unit: "px" });

            const xCursor = gsap.quickTo(lanternRef.current, "x", { duration: 0.1, ease: "power2.out" });
            const yCursor = gsap.quickTo(lanternRef.current, "y", { duration: 0.1, ease: "power2.out" });
            ...
            return () => currentSection.removeEventListener('mousemove', handleMouseMove);
        }, sectionRef);
    ...
    const zones = [
        {
            id: 1,
            title: "My Studies",
            text: "L3 CS Student diving deep into software engineering, algorithms, and system architecture.",
            Icon: PinkLotus,
            className: "zone-left"
        },
        {
            id: 2,
            title: "My Skills",
            text: "React, JavaScript, Linux Ricing. Building highly interactive web experiences.",
            Icon: Frog,
        ...
        },
        {
            id: 3,
            title: "Who I Am",
            text: "A creative developer who believes code should be as beautiful on the screen as it is under the hood.",
            Icon: PinkLotus,
    ...
    return (
        <section id="about" className="enchanted-pond-container" ref={sectionRef}>

            <div className="lantern-cursor" ref={lanternRef}></div>

            <div className="hint-text">
                <span className="blink">˖⋆𓍢ִ໋</span> move your light & click to read <span className="blink">✧˖°.</span>
            </div>

            <div className="flashlight-reveal-layer">
                ...
                {zones.map((zone) => {
                    ...
                    return (
                        <div
                            key={zone.id}
                            className={`interactive-zone ${zone.className} ${isActive ? 'active' : ''} ${isDimmed ? 'dimmed' : ''}`}
